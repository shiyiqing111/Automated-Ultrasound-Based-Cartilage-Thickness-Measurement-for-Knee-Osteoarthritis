...
def plot_comparison_bars_compact(summary_dict, metrics=("MAE", "RMSE", "R2")):
    ...
    colors = {"resnet18": "#4C78A8", "resnet50": "#F58518"}
    hatches = {"resnet18": "", "resnet50": "//"}
    ...
    def _add_value_labels(ax, rects, fmt="{:.2f}"):
        for r in rects:
            height = r.get_height()
            ax.text(r.get_x() + r.get_width() / 2, height,
                    fmt.format(height),
                    ha="center", va="bottom", fontsize=10, color="#333", rotation=0)
    ...
    for metric in metrics:
        ...
        for i, m in enumerate(models):
            ...
            ax.set_xticks(x)
            ax.set_xticklabels([s.capitalize() for s in segments])
            ax.set_title(model_titles[m], pad=6)
            ax.set_ylabel(metric if i in (0, 2) else "")
